...
class JDPhotoBrowserCell: UICollectionViewCell,UIGestureRecognizerDelegate ,UIScrollViewDelegate{
    ...
    var imageUrl: String?{
        didSet{
            ...
            self.backImg.sd_internalSetImage(with: URL(string: imageUrl!), placeholderImage: placeImage, options: SDWebImageOptions.progressiveDownload, operationKey: nil, setImageBlock: nil, progress: nil) { (image, _, _, _) in
                self.getImageSize(image: image!)
            }
        }
    }
