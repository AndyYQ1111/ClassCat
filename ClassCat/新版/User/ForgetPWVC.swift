...
class ForgetPWVC: BaseViewController {
    ...
    @IBAction func sureAction(_ sender: UIButton) {
        if(tf_phonenum.text?.count == 0 || tf_code.text?.count==0 || tf_pw1.text?.count==0||tf_pw2.text?.count==0){
            ...
            let paraDic:[String:Any] = ["userPhone" : tf_phonenum.text ?? "","smsCode" : tf_code.text ?? "","timeStamp":timeStamp!,"password":tf_pw1.text ?? ""]
