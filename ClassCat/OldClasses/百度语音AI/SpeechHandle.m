...
-(void)noteRecordPath:(NSNotification*)note {
    int op = [[note object] intValue];
    //AI功能：  0:speex开始 1:speex结束 2:手机录音结束
    //录音功能： 4:设备上『录音功能』录音结束
    if (op == 0 || op == 4) {
        return;
    }
...
-(void)initTTS{
    ...
    [authConfigDic setObject:@"278578411" forKey:K_PRODUCT_ID];//用户产品ID
    [authConfigDic setObject:@"7be0a0a2480f72d1be2eb9e65c3ac646" forKey:K_API_KEYS];//用户授权key
