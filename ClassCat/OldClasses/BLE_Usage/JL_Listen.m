...
@interface JL_Listen(){
    ...
    int devBtnType; //设备上按钮的类型0：表示AI键，109表示录音键
}
@end
...
-(void)noteSpeexStart:(NSNotification*)note{
    ...
    //设备上按钮的类型0：表示AI键，109表示录音键
    devBtnType = [arr[1] intValue];
    NSLog(@"inputType==%d---num_1==%d",inputType,devBtnType);
    ...
    if (inputType == 0) {
        if (recordType == 0) {
            ...
            [DFAction mainTask:^{
                /*--- UI正在识别 0表示AI---*/
                [DFNotice post:@"speech_start" Object:@"0"];
            }];
        }
    }
...
//            __weak typeof(self) weakSelf = self;
            [DFAction mainTask:^{
                /*--- UI正在识别 1表示录音---*/
                if(self->devBtnType==0){
                    [DFNotice post:@"speech_start" Object:@"0"];
                }else{
                    [DFNotice post:@"speech_start" Object:@"1"];
                }
            }];
        }
    }
...
-(void)noteSpeexStop:(NSNotification*)note{
    ...
    if(self->devBtnType==0){
        ...
        //录音按钮录音结束
        [DFNotice post:kJL_RECORD_PATH Object:@(4)];
    }
}
...
-(void)noteConfigInfo:(NSNotification*)note{
    ...
    [DFNotice post:@"kUI_IS_CONFIG" Object:nil];
